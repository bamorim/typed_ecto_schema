defmodule TypedEctoSchema do
  ...
  It ends up in something like this:
    ...
    This is problematic for a lot of reasons, summing up:
  ...
  - `:null` - Set the default `:null` field option, which normally is true. Note that it is still
    ...
    Also, `embeds_many` and `has_many` can never be null, because they are always initialized to an
    empty list, so they never receive the `| nil` on the typespec.
  ...
  - `:additional_types` - (Experimental) When `true`, defines a public named type for each
    ...
    See the [Experimental Features](experimental.html) guide.
  ...
  Two opt-in features are documented in the [Experimental Features](experimental.html) guide:

  - **Additional named types**: `additional_types: true` generates a public named type for
    each `Ecto.Enum` (and polymorphic embed) field.
  - **PolymorphicEmbed integration**: `polymorphic_embeds_one/2` and
    `polymorphic_embeds_many/2` support behind a compile-time flag.
